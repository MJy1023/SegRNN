...
#DH-FNN layer
class spike_dense_test_denri_nospike(nn.Module):
    def __init__(self,input_dim,output_dim,tau_minitializer = 'uniform',low_m = 0,high_m = 4,
                 tau_ninitializer = 'uniform',low_n = 0,high_n = 4,vth = 0.5,dt = 1,branch = 4,device='cpu',
                 bias=True,test_sparsity = False,sparsity=0.5,mask_share=1):
        """
        Args:
            input_dim(int): input dimension.
            output_dim(int): the number of readout neurons
        ...
        self.pad = ((input_dim)//branch*branch+branch-(input_dim)) % branch
        self.dense = nn.Linear(input_dim+self.pad, output_dim*branch)
        #! 此处修改device的使用，使之隐式表示，这样整个spike_rnn_test_denri_nospike就可以 .to(device) 了   0507 mjy 
        self.device = self.dense.weight.device  
        #sparsity
        self.overlap = 1/branch
        self.tau_m = nn.Parameter(torch.Tensor(self.output_dim))
        self.tau_n = nn.Parameter(torch.Tensor(self.output_dim,branch))
    ...
    def forward(self,input_spike):
        ...
        #update membrane potential and generate spikes
        self.mem,self.spike = mem_update_para_nospike(l_input,self.mem,self.spike,self.v_th,self.tau_m,self.dt,device=self.device)
